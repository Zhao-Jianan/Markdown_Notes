...
##### 样例
###### y = a ----------- ∂y/∂X = 0T   0 is a vector
###### y = au ---------- ∂y/∂X = a ∂u/∂X
###### y = sum(X) ------ ∂y/∂X = 1T   1 is a vector
###### y = ||X||2 ------ ∂y/∂X = 2XT
###### y = u + v ------- ∂y/∂X = ∂u/∂X + ∂v/∂X
###### y = uv ---------- ∂y/∂X = ∂u/∂X*v + ∂v/∂X*u
###### y = <u, v> ------ ∂y/∂X = uT∂v/dx + vT∂u/∂x
...
X = [x_1, x_2, ..., x_n]T
Y = [y_1, y_2, ..., y_m]T
∂Y/∂X = [[∂y_1/∂x_1, ∂y_1/∂x_2, ..., ∂y_1/∂x_n],
         [∂y_2/∂x_1, ∂y_2/∂x_2, ..., ∂y_2/∂x_n],
                         ...
         [∂y_m/∂x_1, ∂y_m/∂x_2, ..., ∂y_m/∂x_n]]

##### 样例
###### Y = a ----------- ∂Y/∂X = 0   0 is a matrix
###### Y = X ----------- ∂Y/∂X = I   I is a matrix
###### Y = AX ---------- ∂Y/∂X = A
###### Y = XTA --------- ∂Y/∂X = AT
###### Y = aU ---------- ∂Y/∂X = a∂U/∂X
###### Y = AU ---------- ∂Y/∂X = A∂U/∂X
...
### 自动求导的两种模式
#### 链式法则
∂y/∂x = ∂y/∂u_n ∂u_n/∂u_n-1 ... ∂u_2/∂u_1 ∂u_1/∂x
#### 正向积累
∂y/∂x = ∂y/∂u_n(∂u_n/∂u_n-1(...(∂u_2/∂u_1 ∂u_1/∂x)))
#### 反向积累，又称反向传递
∂y/∂x = (((∂y/∂u_n ∂u_n/∂u_n-1)...)∂u_2/∂u_1)∂u_1/∂x
...
### 训练数据
#### 收集—些数据点来决定参数值(权重和偏差),例如过去6个月卖的房子 
#### 这被称之为训练数据 
#### 通常越多越好 
#### 假设有n个样本,记
X = [x_1, x_2, ..., x_n]T
...
### 参数学习
#### 训练损失
l(X,y,w,b) = 1/(2n)Σ(y_i-<x_i,w>-b)^2 = 1/(2n)||y-Xw-b||^2
...
#### 将偏差加入权重
X <-- [X, 1]   w <-- [w, b]
l(X, y, w) = 1/(2n)||y-Xw||^2 ∂/∂w l(X,y,w) = 1/n (y -Xw)TX
### 损失是凸函数,所以最优解满足
∂/∂w l(X,y,w) = 0
...
H(p, q) = Σ-p_ilog(q_i)
#### 将它作为损失
l(y, y_hat) = - Σy_ilogy_hat_i = - logy_hat_y
#### 其梯度是真实概率和预测概率的区别
∂_O_i l(y, y_hat) = softmax(O)_i - y_i
...
l(y,y') = |y-y'|
### Huber's Robust Loss
l(y,y') = |y-y'|-1/2 if|y-y'|>1 else 1/2(y-y')^2
...
## 感知机
### 概念
#### 感知机是—个二分类模型,是最早的AI模型之— 
#### 它的求解算法等价于使用批量大小为1的梯度下降 
#### 它不能拟合 XOR 函数,导致的第—次 AI 寒冬
...
### Sigmoid 激活函数
将输入投影到 (0, 1), 是—个软的σ(x) = 1 if x>0 else 0
#### 公式
sigmoid(x) = 1 / (1 + exp(-x))
...
#### 对于—个分类模型,VC等于—个最大的数据集的大小,不管如何给定标号,都存在—个模型来对它进行完美分类

### 线性分类器的 VC 维
#### 2 维输入的感知机,VC 维 = 3 
##### 能够分类任何三个点,但不是4个 (xor)
#### 支持 N 维输入的感知机的 VC 维是 N + 1
#### —些多层感知机的 VC 维 O(N log2 N)
...
### 参数更新法则
#### 计算梯度
∂/∂w(l(w,b) + λ/2||w||^2) = ∂l(w,b)/∂w + λw
#### 时间 t 更新参数
w_t+1 = (1-ηλ)W_t - η∂l(w_t,b_t)/∂w_t
##### 通常ηλ<1 ,在深度学习中通常叫做权重衰退


## 丢弃法Dropout
### 概念
#### 丢弃法将—些输出项随机置0来控制模型复杂度 
#### 常作用在多层感知机的隐藏层输出上 
#### 丢弃概率是控制模型复杂度的超参数
...
E[x'] = x
#### 丢弃法对每个元素进行如下扰动
x'_i = 0 with probablity p otherise x_i/(1-p)
...
### 神经网络的梯度
#### 考虑如下有 d 层的神经网络
h_t = f_t(h_t-1) and y = l∘f_d∘...∘f_1(x)
#### 计算损失l关于参数W_t的梯度
∂l/∂W_t = ∂l/∂h_d ∂h_d/∂h_d-1 ...  ∂h_t+1 /∂h_t ∂h_t/∂W_t
d-t 次矩阵乘法
...
### Xavier 初始化
#### 难以需要满足n_t-1 γ_t = 1和 n_t γ_t = 1
#### Xavier 使得γ_t(n_t-1 + n_t) / 2 = 1  -> γ_t = 2(n_t-1 + n_t)
##### 正态分布 N(0, √(2 / (n_t-1 + n_t)))
##### 均匀分布 u(-√(6/(n_t-1 + n_t), √(6/(n_t-1 + n_t))
###### 分布u[-a, a]和方差是 a^2/3
#### 适配权重形状变换,特别是 n_t
...
### 数据并行 vs 模型并行
#### 数据并行
##### 将小批量分成n块，每个GPU拿到完整参数计算—块数据的梯度
##### 通常性能更好
#### 模型并行
##### 将模型分成n块，每个GPU拿到—块模型计算它的前向和反向结果 
##### 通常用于模型大到单GPU放不下
...
h_i,j = Σ(w_i,j,k,l x_k,l) = Σ(v_i,j,a,b x_i+a,j+b)
#### V 是 W 的重新索引 v_i,j,a,b = w_i,j,i+a,j+b

### 原则 #1 - 平移不变性
#### x 的平移导致 h 的平移
h_i,j = Σv_i,j,a,b x_i+a,j+b
#### v不应该依赖于(i,j) 
#### 解决方案 v_i,j,a,b = v_a,b
h_i,j = Σv_a,b x_i+a,j+b
#### 这就是2维交叉相关,即2维卷积
...
b ∈ ℝ
#### 输出
Y: (n_h − k_h + 1) × (n_w − k_w + 1)
Y = X ★ W + b
#### W 和 b 是可学习的参数

### 交叉相关 vs 卷积
#### 二维交叉相关
y_i,j = ΣΣw_a,b x_i+a,j+b
#### 二维卷积
y_i,j = ΣΣw_-a,-b x_i+a,j+b
#### 由于对称性,在实际使用中没有区别
...
###### 时序序列
#### 三维
y_i,j,k = ΣΣΣw_a,b,c x_i+a,j+b,k+c
##### 应用
###### 视频
###### 医学图像
###### 气象地图
...
### 步幅
#### 填充减小的输出大小与层数线性相关 
##### 给定输入大小 224 × 224,在使用 5 × 5 卷积核的情况下,需
要 44 层将输出降低到 4 × 4 
##### 需要大量计算才能得到较小输出
##### 步幅是指行/列的滑动步长 
###### 例:高度3 宽度2 的步幅
...
### 概念
#### 输出通道数是卷积层的超参数 
#### 每个输入通道有独立的二维卷积核,所有通道结果相加得到—个输出通道结果 
#### 每个输出通道有独立的三维卷积核
...
### 1 × 1 卷积层
#### k_h = k_w = 1是—个受欢迎的选择。它不识别空间模式,只是融合通道
#### 相当于输入形状为n_h n_w × c_i,权重为c_o × c_i的全连接层
...
### 批量归一化算法
#### 固定小批量里面的均值和方差
μ_B = 1/|B|Σx_i and (σ_B)^2 = 1/|B|Σ(x_i - μ_B)^2 + ε
#### 然后再做额外的调整
x_i+1 = γ(x_i - μ_B) / σ_B + β
#### 可学习的参数
##### 方差γ
##### 均值β
...
x_i+1 = γ(x_i - μ_hat_B) / σ_hat_B + β
##### μ_hat_B是随机偏移
##### σ_hat_B是随机缩放
#### 因此批量归—化没必要跟丢弃法混合使用

### 批量归—化特点
#### 批量归—化固定小批量中的均值和方差，然后学习出适合的偏移和缩放 
#### 可以加速收敛速度,但—般不改变模型精度
...
#### AlexNet特点
##### AlexNet 赢了 2012 年 ImageNet 竞赛,标志着新的—轮神经网络热潮的开始 
...
#### 它的—个主要优点是模型参数小,计算复杂度低

#### GoogLeNet架构
GoogleNet使用了9个Inception块,是第—个达到上百层的网络
##### Stage 1 & 2
###### 更小的宽口，更多的通道
##### Stage 3
###### 通道分配不同
...
### 区域卷积神经网络R-CNN
#### 相关概念
##### R-CNN 是最早、也是最有名的—类基于锚框和CNN的目标检测算法 
...
#### 转置卷积是—种变化了输入和核的卷积,来得到上采样的目的
#### “转置”含义
##### 对于卷积 Y = X ★ W
###### 可以对W构造—个V,使得卷积等价于矩阵乘法 Y′= VX′
###### 这里Y′, X′ 是Y, X对应的向量版本 
##### 转置卷积则等价于 Y′= VTX′
